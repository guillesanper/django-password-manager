        ...
        if master_password:
            ...
            if master_key_entry.verify_master_key(master_password):
                ...
                decrypted_password = decrypt_password(
                    ...
                    master_key=master_key_entry.hashed_key.encode(),  # Eliminar el encoding si ya está en bytes
                    entry_salt=account.salt.encode(),  # Asegúrate de pasar la salt correcta y en formato adecuado
                    algorithm=account.encryption_algorithm
                )
                account.decrypted_password = decrypted_password  # Añade esta propiedad temporalmente
            else:
